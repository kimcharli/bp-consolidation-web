...
from .ck_global import global_store, sse_queue, SseEvent, SseEventEnum, SseEventData, sse_logging, GlobalStore
...
async def upload_env_ini(request: Request, file: UploadFile):
    ...
    await sse_logging(f"/upload-env-ini begin")
    ...
    await sse_logging(f"/upload-env-ini end")
...
async def login_tor_bp(request: Request):
    global global_store
    global_store.stop_signal = True
    await global_store.reset_tor_data()
    # global_store.stop_signal = False

    tor_bp = request.query_params['tor-bp']
    await sse_logging(f"/login-tor-bp login {tor_bp}")
...
async def disconnect():
    ...
    await sse_logging(f"/disconnect")
    global_store.logout_server()
    await global_store.reset_tor_data()
    # global_store.stop_signal = False

    # enable load button again
    await SseEvent(data=SseEventData(id='upload-env-ini-input', just_value='')).send()
...
async def sync():
    ...
    global_store.stop_signal = False
    await sse_logging(f"/sync begin")
    ...
    await ctWorker.sync_tor_ct()
    await ctWorker.sync_main_ct()
    await ctWorker.referesh_ct_table()

    await global_store.migration_status.set_sync_done()

    await sse_logging(f"/sync end")
    return 'sync done'
...
async def migrate_access_switches():
    ...
    await sse_logging(f"/migrate_access_switches begin")
    ...
    await sse_logging(f"/migrate_access_switches end")
...
async def migrate_generic_system():
    ...
    global_store.stop_signal = False
    await sse_logging(f"/migrate_generic_systems begin")
    await SseEvent(data=SseEventData(id=SseEventEnum.BUTTON_MIGRATE_GS).loading()).send()
    await global_store.genericSystemWorker.migrate_generic_systems()
    await sse_logging(f"/migrate_generic_systems end")
    return {}
...
async def migrate_virtual_networks():
    ...
    await sse_logging(f"/migrate_virtual_networks begin")
    await global_store.virtualNetworks.migrate_virtual_networks()
    await sse_logging(f"/migrate_virtual_networks end")
    return {}
...
async def migrate_cts():
    ...
    global_store.stop_signal = False
    await SseEvent(data=SseEventData(id=SseEventEnum.BUTTON_MIGRATE_CT).loading()).send()
    CtWorker = global_store.ctWorker
    await sse_logging(f"/migrate_cts begin")
    await CtWorker.migrate_connectivity_templates()
    await sse_logging(f"/migrate_cts end")
    return {}
...
async def compare_config():
    ...
    global_store.stop_signal = False
    await sse_logging(f"/compare_config begin")
    await global_store.compare_config()
    await sse_logging(f"/compare_config end")
